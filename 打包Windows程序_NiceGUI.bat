...
set "LOG_FILE=%~dp0build_windows_nicegui.log"
set "PY_CMD="

> "%LOG_FILE%" echo [INFO] Start NiceGUI build at %date% %time%
>> "%LOG_FILE%" echo [INFO] Workdir: %cd%

title Attendance Tool NiceGUI Build
echo ==============================
echo Attendance Tool NiceGUI Build
echo ==============================
echo.
echo Log file:
echo %LOG_FILE%
...
%PY_CMD% -m pip install --upgrade pip pyinstaller nicegui pandas openpyxl xlrd holidays chinese-calendar >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
...
echo [3/4] Building NiceGUI exe...
%PY_CMD% -m PyInstaller --noconfirm --clean attendance_nicegui.spec >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
...
echo Output folder:
echo %~dp0dist
>> "%LOG_FILE%" echo [OK] NiceGUI build completed.
echo.
...
:error
echo.
echo [ERROR] NiceGUI build failed.
